	...
	static void AddLevItem(TESObjectREFR* ref, TESForm* form, UInt32 amount) {
		...
		while (itr > 0) {
			...
			if (newForm) {
				...
				if (newForm->GetFormType() == kFormType_Ammo ||
					newForm->GetFormType() == kFormType_Clothing ||
					newForm->GetFormType() == kFormType_Armor ||
					newForm->GetFormType() == kFormType_Weapon) {
					Manager::GetSingleton()->QueueEquip(ref->refID, form->refID);
				}
			}
			else if (lev->chanceNone == 0) {
				newForm = lev->list.Info()->form;
				if (newForm) {
					ref->AddItem(newForm, nullptr, 1);
					if (newForm->GetFormType() == kFormType_Ammo ||
						newForm->GetFormType() == kFormType_Clothing ||
